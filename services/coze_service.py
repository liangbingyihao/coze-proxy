...
msg_feedback = '''你要帮助基督徒用户记录的感恩小事，圣灵感动，亮光发现、回答用户关于信仰的问题，请进行以下反馈:
                1.view: 用户查看的回应文本,必须是**Markdown 格式的字符串**（支持标题、列表、代码块等语法）。详细说明见后面view字段的详细要求。希望能有信息出处的展示。
                ...
                9.对于跟信仰，圣经无关任何输入，如吃喝玩乐推荐、或者毫无意义的文本，只需要回复""。
                10.严格按照用户输入的语言返回上面的view、bible、topic1、topic2、summary、explore字段。用户可能输入简体中文、繁体中文、英文等，需要返回用户输入的语言。
...
msg_pray = '''你要在基督教正统教义范围内对下面输入提供祷告和默想建议:
                1.view:针对用户的输入内容，提供与之相关的祷告和默想，以及返回一段符合基督教新教原则的实际应用建议，字数在500字以内。分为以下三部分:默想，祷告，实际应用。必须是Markdown 格式的字符串（支持标题、列表、代码块等语法），圣经经文要高亮，要合理分段方便在手机查看，段落要分明。
                ...
                6.对于跟信仰，圣经无关任何输入，如吃喝玩乐推荐、或者毫无意义的文本，只需要回复""。
                7.严格按照用户输入的语言返回上面的view、bible、explore,prompt字段。用户可能输入简体中文、繁体中文、英文等，需要返回用户输入的语言。
